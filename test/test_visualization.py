...
import emotion.visualization as visualization_module


class TestVisualizationModuleGetattr(unittest.TestCase):
    """Test __getattr__ in visualization module"""

    def test_getattr_popane_figure_generator(self):
        """Test __getattr__ returns POPANEFigureGenerator"""
        result = visualization_module.__getattr__('POPANEFigureGenerator')
        self.assertEqual(result, POPANEFigureGenerator)

    def test_getattr_version(self):
        """Test __getattr__ returns __version__"""
        result = visualization_module.__getattr__('__version__')
        self.assertIsNotNone(result)

    def test_getattr_unknown(self):
        """Test __getattr__ returns None for unknown attribute"""
        result = visualization_module.__getattr__('unknown_attr')
...
class TestCreateFigureForSubject(unittest.TestCase):
    """Test create_figure_for_subject method"""
    ...
    def setUp(self):
        ...
        self.sample_df = pd.DataFrame({
            'SUBJECT_ID': [1] * 100 + [2] * 100,
            'STUDY_NAME': ['Study1'] * 200,
            'EMOTION': ['happy'] * 50 + ['sad'] * 50 + ['happy'] * 50 + ['sad'] * 50,
            'timestamp': list(range(50)) * 4,
            'ECG': np.random.randn(200),
            'EDA': np.random.randn(200),
            'SBP': np.random.randn(200) + 120,
            'DBP': np.random.randn(200) + 80,
        })
    ...
    def test_create_figure_returns_figure(self):
        """Test that create_figure_for_subject returns a Figure"""
        fig = POPANEFigureGenerator.create_figure_for_subject(
            self.sample_df,
            subject_id=1,
            start_end=(0, 10)
        )
    ...
    def test_create_figure_no_subject_returns_none(self):
        """Test that nonexistent subject returns None"""
        result = POPANEFigureGenerator.create_figure_for_subject(
            self.sample_df,
            subject_id=999  # Nonexistent
        )

        self.assertIsNone(result)

    def test_create_figure_with_custom_figsize(self):
        """Test creating figure with custom figsize"""
        fig = POPANEFigureGenerator.create_figure_for_subject(
            self.sample_df,
            subject_id=1,
            figsize=(20, 16),
    ...
    def test_create_figure_with_different_time_ranges(self):
        """Test creating figure with different time ranges"""
        fig = POPANEFigureGenerator.create_figure_for_subject(
            self.sample_df,
            subject_id=1,
            start_end=(5, 40)
...
class TestCreateFigureOnePerStudy(unittest.TestCase):
    """Test create_figure_one_per_study method"""
    ...
    def setUp(self):
        ...
        self.study_data_dict = {
            'study1_subject1': pd.DataFrame({
                'Subject_ID': [1] * 50,
                'SUBJECT_ID': [1] * 50,
                'STUDY_NAME': ['Study1'] * 50,
                'EMOTION': ['happy'] * 25 + ['sad'] * 25,
                'timestamp': list(range(50)),
                'ECG': np.random.randn(50),
                'EDA': np.random.randn(50),
            }),
            'study2_subject1': pd.DataFrame({
                'Subject_ID': [2] * 50,
                'SUBJECT_ID': [2] * 50,
                'STUDY_NAME': ['Study2'] * 50,
                'EMOTION': ['angry'] * 25 + ['fear'] * 25,
                'timestamp': list(range(50)),
                'ECG': np.random.randn(50),
                'EDA': np.random.randn(50),
            })
    ...
    def test_create_figures_returns_list(self):
        """Test that create_figure_one_per_study returns list of figures"""
        figures = POPANEFigureGenerator.create_figure_one_per_study(
            self.study_data_dict,
            start_end=(0, 10)
        )
    ...
    def test_create_figures_returns_figure_instances(self):
        """Test that returned list contains Figure instances"""
        figures = POPANEFigureGenerator.create_figure_one_per_study(
            self.study_data_dict,
            start_end=(0, 10)
        )
    ...
    def test_create_figures_empty_dict_returns_empty_list(self):
        """Test with empty dict returns empty list"""
        figures = POPANEFigureGenerator.create_figure_one_per_study({})
