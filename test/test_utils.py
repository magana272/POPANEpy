...
class TestGetSize(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for study_num in range(1, 8):
            study_dir = os.path.join(self.test_dir, f'S{study_num}')
            os.makedirs(study_dir, exist_ok=True)
            
            # Create a few CSV files
            for i in range(3):
                filepath = os.path.join(study_dir, f'S{study_num}_subject{i}.csv')
    ...
    def test_get_size_counts_files(self):
        ...
        for i in range(1, 8):
            self.assertEqual(result[f'S{i}']['File_Count'], 3)
